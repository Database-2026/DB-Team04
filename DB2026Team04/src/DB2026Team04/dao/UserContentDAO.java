...
public class UserContentDAO {
    ...
    public int getPcIdByTitle(String title) {
        // Content 테이블과 PlatformContent 테이블을 content_id로 조인하여 pc_id를 추출합니다.
        String sql = "SELECT pc.pc_id " +
                     "FROM Content c " +
                     "JOIN PlatformContent pc ON c.content_id = pc.content_id " +
                     "WHERE c.title LIKE ? AND pc.is_available = true " +
                     "LIMIT 1"; // 여러 플랫폼에 있다면 첫 번째 플랫폼의 pc_id를 가져옴
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt("pc_id"); // 스키마에 맞는 pc_id 리턴
                }
            }
        } catch (Exception e) {
            System.out.println("콘텐츠(pc_id) 조회 중 오류 발생: " + e.getMessage());
        }
        
        return -1; // 매칭되는 콘텐츠나 사용 가능한 플랫폼이 없을 때
    }
}
